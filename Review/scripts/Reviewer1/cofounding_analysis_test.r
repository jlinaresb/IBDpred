...
# ===
clin = as.data.frame(test@sam_data)
...
test.df = t(test@otu_table)
colnames(test.df) = paste0('X', colnames(test.df))
test.df = test.df[, features]
test.df = apply(test.df, 2, function(x) log2(x + 1))
test.df = cbind.data.frame(test.df, target = get_variable(test, 'IBD'))
test.task = makeClassifTask(data = test.df, target = 'target')
test.task = normalizeFeatures(test.task)

pred = predict(best, test.task, type = 'prob')
